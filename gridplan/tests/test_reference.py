import numpy as np
import pytest
from gridplan.reference import dijkstra, a_star
...
class TestAStar:
    ...
    def test_start_equals_target(self):
        grid = np.zeros((10, 10), dtype=np.uint8)
        result = a_star(grid, (5, 5), (5, 5))
        assert result.cost == 0
        assert result.path == [(5, 5)]

    def test_corner_to_corner_empty_grid(self):
        grid = np.zeros((10, 10), dtype=np.uint8)
        result = a_star(grid, (0, 0), (9, 9))
        assert result.cost == 18
        assert result.path[0] == (0, 0)
        assert result.path[-1] == (9, 9)
    ...
    def test_cost_matches_dijkstra(self):
        """A* with an admissible heuristic must find the same optimal cost as Dijkstra."""
        grid = np.zeros((10, 10), dtype=np.uint8)
        grid[2, 0:8] = 1
        grid[5, 2:10] = 1
        grid[7, 0:6] = 1
        d_result = dijkstra(grid, (0, 0), (9, 9))
        a_result = a_star(grid, (0, 0), (9, 9))
        assert a_result.cost == d_result.cost

    def test_fewer_expansions_than_dijkstra(self):
        """A* should generally expand fewer nodes than Dijkstra on the same grid."""
        grid = np.zeros((10, 10), dtype=np.uint8)
        d_result = dijkstra(grid, (0, 0), (9, 9))
        a_result = a_star(grid, (0, 0), (9, 9))
        assert a_result.n_nodes_expanded <= d_result.n_nodes_expanded
